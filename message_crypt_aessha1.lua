...
cryptor.encrypt = function(message)
    ...
    if ok then        
        ...
        hash = encoder.toHex(crypto.hmac('sha1', iv..json, ENCODER_AESSHA1_PASSPHRASE)) 
        message["event"] = cryptor.name
        message["parameters"] = {
            encoder.toHex(encrypted_iv), 
            encoder.toHex(encrypted_data), 
            hash
        }
        message["response" ] = ""
    end
...
cryptor.decrypt = function(message)    
    ...
    data = data:gsub("%z","")
    calculated_hash = encoder.toHex(crypto.hmac('sha1', iv..data, ENCODER_AESSHA1_PASSPHRASE))
